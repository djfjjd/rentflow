...
export async function onRequestPost({ request, env }: { request: Request; env: Env }) {
  try {
    ...
    const { results } = await env.DB.prepare("SELECT endpoint FROM push_subscriptions ORDER BY created_at DESC").all();
    ...
    return Response.json({
      ...
      note: "Web Push delivery requires VAPID signing. Subscription storage and client notification hooks are ready.",
    });
  } catch (error) {
