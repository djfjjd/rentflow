...
export async function onRequestPost({ request, env }: { request: Request, env: Env }) {
  try {
    ...
    const result = await env.DB.prepare(
      ...
      safeNullableText(f.r2ThumbnailKey || f.r2_thumbnail_key || f.thumbnailKey),
      safeText(f.driveBackupStatus || "none"),
      safeNullableText(f.driveFileId || f.googleDriveFileId),
      safeNullableText(f.driveUrl || f.googleDriveViewUrl),
