...
export async function onRequestPost({ request, env }: { request: Request, env: Env }) {
  try {
    ...
    await env.DB.prepare(
      "INSERT INTO returns (id, rental_car_number, return_address, arrival_address, fuel_level, fuel_display, mileage, notes, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
    ).bind(
      r.id, r.rentalCarNumber, r.returnAddress, r.arrivalAddress, r.fuelLevel || null, r.fuelDisplay || null, r.mileage || 0, r.notes, r.status || "회차등록"
    ).run();
